...
async def _schema() -> AsyncIterator[None]:
    ...
    async with engine.begin() as conn:
        ...
        # `create_all` only builds what the ORM models declare — the
        # `work_done_shift_uniqueness` trigger (migration 0028) is raw SQL,
        # so it's mirrored here the same way the trgm index above is.
        await conn.execute(
            text(
                """
                CREATE OR REPLACE FUNCTION check_work_done_shift_uniqueness()
                RETURNS trigger AS $$
                DECLARE
                    conflict_id varchar;
                BEGIN
        ...
        await conn.execute(
            text(
                "DROP TRIGGER IF EXISTS trg_work_done_shift_uniqueness "
                "ON work_done_entries"
            )
        )
        await conn.execute(
            text(
