...
Eight grids, all the same shape: the fleet down the side, the days across the
top, one mark per bus per day. That shape is the point — a month of one thing
for every bus at once is how a gap becomes visible, which no per-entry list
does.

The depot's own instructions carry the colour rules, and they are what the
marks below encode: PM days shaded on the coolant and energy charts,
breakdowns red on their own chart.
"""
...
from app.models.enums import CheckResult, StrEnum
...
class ChartKind(StrEnum):
    ...
    #: D.I and the 10-day service, each its own tick-per-bus-per-day grid —
    #: final now; the two used to be merged into one P.M schedule chart that
    #: could only show one work-type code per day.
    di_inspection = "diInspection"
    ten_day_service = "tenDayService"
    #: Driver complaints and breakdowns, each its own grid — final now; the
    #: two used to be merged, and a breakdown would silently overwrite that
    #: day's complaint count.
